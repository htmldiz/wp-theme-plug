...
window.update_repeater_fields = function (){
    ...
    function init (){
      return {
        ...
        hooks:function(){
          ...
          $('body').on('click',".simplePanelimageUploadclear,.simplePanelfileUploadclear", function( event ){
            event.preventDefault();
...
jQuery(function($) {
  $('body').on('submit', '.theme-options-ont form', function(event) {
    var data = $(this).serialize();
    data = data+'&action=update_theme_options';
    $.ajax({
      url: ajaxurl,
      type: 'POST',
      dataType: 'json',
  ...
  $('body').on('click', '[data-selectorfor]', function(event) {
    window.selector = $(this).attr('data-selectorfor');
    frame.open();
    return false;
  });
  frame.on( 'select', function() {
    ...
    if(window.selector){
      $('#'+window.selector).val( attachment.url );
      if(!$('#'+window.selector).parent().find('img').length > 0){
        $('#'+window.selector).parent().find('.img-place').append('<img src="" alt="" />');
      }
      $('#'+window.selector).parent().find('img').attr('src',attachment.url);
    }
  });
