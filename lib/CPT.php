...
class CPT {
	...
	function register_post_type() {
		...
		$labels = array(
			...
			'singular_name'      => sprintf( __( '%s' ), $singular ),
			'menu_name'          => sprintf( __( '%s' ), $plural ),
	...
	function register_taxonomy($taxonomy_names, $options = array()) {
		...
		$labels = array(
			...
			'separate_items_with_commas' => sprintf( __( 'Seperate %s with commas' ), $plural ),
			'add_or_remove_items'        => sprintf( __( 'Add or remove %s' ), $plural ),
			'choose_from_most_used'      => sprintf( __( 'Choose from most used %s' ), $plural ),
			'not_found'                  => sprintf( __( 'No %s found' ), $plural ),
		);
					...
					if ( is_array( $this->taxonomies ) ) {
						...
						foreach( $this->taxonomies as $tax ) {
							...
							if( $tax !== 'category' && $tax !== 'post_tag' ) {
								...
								$new_columns[ $tax ] = sprintf( __( '%s' ), $taxonomy_object->labels->name );
							}
						}
					}
				}
		...
		switch( $column ) {
			...
			case ( taxonomy_exists( $column ) ) :
				...
				if ( ! empty( $terms ) ) {
					...
					// Echo no terms.
					printf( __( 'No %s' ), $taxonomy_object->labels->name );
				}
			...
			if ( ! empty( $filters ) ) {
				...
				foreach ( $filters as $tax_slug ) {
					...
					if ( $terms ) {
						...
						// Default show all.
						printf( '<option value="0">%s</option>', sprintf( __( 'Show all %s' ), $tax->label ) );
	...
	function updated_messages( $messages ) {
		...
		$messages[$this->post_type_name] = array(
			...
			5 => isset( $_GET['revision'] ) ? sprintf( __( '%2$s restored to revision from %1$s' ), wp_post_revision_title( (int) $_GET['revision'], false ), $singular ) : false,
			6 => sprintf( __( '%s updated.' ), $singular ),
			7 => sprintf( __( '%s saved.' ), $singular ),
			8 => sprintf( __( '%s submitted.' ), $singular ),
			9 => sprintf(
				__( '%2$s scheduled for: <strong>%1$s</strong>.' ),
