...
jQuery(document).ready(function($){
  ...
  if($('.at-field select:not(.at-posts-select)').length > 0){
    $('.at-field select:not(.at-posts-select)').select2();
  }
  if($('.at-field select.at-posts-select').length > 0){
    $('.at-field select.at-posts-select').each(function(index,el){
      var posttype = $(el).attr('data-posttype');
      posturl = '/wp-admin/admin-ajax.php?action=search_post_by_type&post_type='+posttype;
      $(el).select2({
        ajax:{
          url: posturl,
          dataType: 'json'
